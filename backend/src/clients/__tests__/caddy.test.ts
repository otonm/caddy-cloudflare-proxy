...
describe('addRoute', () => {
  ...
  it('falls back to PUT when POST returns 404', async () => {
    const spy = vi
      .spyOn(global, 'fetch')
      .mockResolvedValueOnce({ ok: false, status: 404, text: () => Promise.resolve('not found') } as Response)
  ...
  it('throws when both POST and PUT fail', async () => {
    vi.spyOn(global, 'fetch')
      ...
      .mockResolvedValueOnce({ ok: false, status: 500, text: () => Promise.resolve('error') } as Response)
    const { addRoute } = await import('../caddy')
