...
router.get('/:id/status', async (req, res) => {
  ...
  try {
    ...
    res
      .status(isUpstreamError(err) ? 502 : 500)
      .json({ error: 'Failed to query Caddy', details: err instanceof Error ? err.message : null });
  }
});
...
router.put('/:id', async (req, res) => {
  ...
  const updated: Proxy = {
    ...
    cloudflare: patch.cloudflare
      ? { ...existing.cloudflare, ...patch.cloudflare }
      : existing.cloudflare,
